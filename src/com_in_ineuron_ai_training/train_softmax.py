...
class TrainFaceRecogModel:
    ...
    def trainKerasModelForFaceRecognition(self):
        # Encode the labels
        le = LabelEncoder()
        labels = le.fit_transform(self.data["names"]) ## [array of: names converted into numbers 0,1,2....]
        # print("label",labels)
        num_classes = len(np.unique(labels))
        labels = labels.reshape(-1, 1)
        one_hot_encoder = OneHotEncoder(categorical_features = [0])
        labels = one_hot_encoder.fit_transform(labels).toarray() ## now that array is conterted in to onehot encoding 0-[0. 1. 0.] 1-[1. 0. 0.] 2-[0. 0. 1.]
        # print("labels_onehot",labels)
