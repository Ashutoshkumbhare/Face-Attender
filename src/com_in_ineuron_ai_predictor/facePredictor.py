            ...
            if frames % 3 == 0:
                ...
                if len(bboxes) != 0:
                    ...
                    for bboxe in bboxes:
                        ...
                        # Predict class
                        preds =  self.model.predict(embedding) ## preds [[0.7026396  0.10133359 0.19602682]]
                        # print("preds",preds)
                        preds = preds.flatten() ## preds_flatten [0.7026396  0.10133359 0.19602682]
                        # print("preds_flatten",preds)

                        # Get the highest accuracy embedded vector
                        j = np.argmax(preds) ## argmax 2
                        # print("argmax",j)
                        proba = preds[j]
                        # print("proba",proba)
                        ...
                        #print("match_class_idx",match_class_idx)
                        match_class_idx = np.where(match_class_idx)[0]
                        #print(np.where(match_class_idx))
                        #print("match_class_idx[0]", match_class_idx)
                        selected_idx = np.random.choice(match_class_idx, comparing_num)
                        #print("selected_idx",selected_idx)
                        compare_embeddings = self.embeddings[selected_idx]
                        #print("compare_embeddings",compare_embeddings)
